...
% % % %----------------------------------------------------------------------------------------------------------------------------% % % %
STH=[HHLAT_MA,HHLAT_AM,HHLAT_MJ,HHLAT_MAM,...
    HHLAT_AMJ,HHLAT_MAMJ,...
    HHLON_MA,HHLON_AM,HHLON_MJ,...
    HHLON_MAM,HHLON_AMJ,HHLON_MAMJ,...
    HHPRS_MA,HHPRS_AM,HHPRS_MJ,...
    HHPRS_MAM,HHPRS_AMJ,...
    HHPRS_MAMJ,SPLAT_MA,SPLAT_AM,SPLAT_MJ,SPLAT_MAM,...
    SPLAT_AMJ,SPLAT_MAMJ,...
    SPLON_MA,SPLON_AM,SPLON_MJ,...
    SPLON_MAM,SPLON_AMJ,SPLON_MAMJ,...
    SPPRS_MA,SPPRS_AM,SPPRS_MJ,...
    SPPRS_MAM,SPPRS_AMJ,...
    SPPRS_MAMJ,SALAT_MA,SALAT_AM,SALAT_MJ,SALAT_MAM,...
    SALAT_AMJ,SALAT_MAMJ,...
    SALON_MA,SALON_AM,SALON_MJ,...
    SALON_MAM,SALON_AMJ,SALON_MAMJ,...
    SAPRS_MA,SAPRS_AM,SAPRS_MJ,...
    SAPRS_MAM,SAPRS_AMJ,...
    SAPRS_MAMJ,AZLAT_MA,AZLAT_AM,AZLAT_MJ,AZLAT_MAM,...
    AZLAT_AMJ,AZLAT_MAMJ,...
    AZLON_MA,AZLON_AM,AZLON_MJ,...
    AZLON_MAM,AZLON_AMJ,AZLON_MAMJ,...
    AZPRS_MA,AZPRS_AM,AZPRS_MJ,...
    AZPRS_MAM,AZPRS_AMJ,...
    AZPRS_MAMJ];
...
for h=1:40 %The variable h is assigned to a number representing the hth-year being forecasted in the 40-year analysis.%
    ...
    for i=1:72 %Counter i iterates through the STH list and chooses the first variable being tested as a predictor in the set.%
        ...
        if round(abs(C1(1,2)),2)>=0.42 %The correlation between Variable1 and ENSO must meet the threshold (to check the variable's predictive power of ENSO over the past 30 years), or the loop moves to the next variable.%
            for j=1:72 %Counter j iterates through the STH list and chooses the first variable being tested as a predictor in the set.%
